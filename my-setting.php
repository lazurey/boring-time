<?php
  ...
  if (isset($_POST['user_id']) || isset($_POST['user_img'])) {
    ...
    if ($user_id != "" && $user_img != "unchanged") {
      $update_flag = 1; //两项都变
      $sql = "UPDATE bt_user SET name = '" . $user_id . "', img = '" . $user_img . ".png' WHERE uid = " . $_COOKIE['uid'];
      $flag = true;
    } else if ($user_id != "" && $user_img == "unchanged") {
      $update_flag = 2; //只更改昵称
      $sql = "UPDATE bt_user SET name = '" . $user_id . "' WHERE uid = " . $_COOKIE['uid'];
      $flag = true;
    } else if ($user_id == "" && $user_img != "unchanged") {
      $update_flag = 3; //只更改头像
      $sql = "UPDATE bt_user SET img = '" . $user_img . ".png' WHERE uid = " . $_COOKIE['uid'];
    }
    if ($update_flag != 0) {
